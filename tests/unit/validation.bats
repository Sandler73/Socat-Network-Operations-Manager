...
@test "validate_port_range: rejects inverted range (8010-8000)" {
    run validate_port_range "8010-8000"
    [ "$status" -eq 1 ]
}

@test "validate_port_range: accepts single-port range (8080-8080) (F-08)" {
    run validate_port_range "8080-8080"
    [ "$status" -eq 0 ]
    # A single-port range yields exactly one port.
    local line_count
    line_count="$(echo "${output}" | wc -l)"
    [ "${line_count}" -eq 1 ]
    [ "${output}" = "8080" ]
...
# --- S-02: keyword allowlist (reject dangerous/unknown options) --------

@test "validate_socat_opts: rejects exec option (program spawn)" {
    run validate_socat_opts "reuseaddr,exec=/bin/sh"
    [ "$status" -eq 1 ]
}

@test "validate_socat_opts: rejects system option (shell spawn)" {
    run validate_socat_opts "system=id"
    [ "$status" -eq 1 ]
}

@test "validate_socat_opts: rejects shell option" {
    run validate_socat_opts "shell"
    [ "$status" -eq 1 ]
}

@test "validate_socat_opts: rejects unknown keyword" {
    run validate_socat_opts "fork,definitelynotanoption"
    [ "$status" -eq 1 ]
}

@test "validate_socat_opts: accepts a mix of bare and key=value allowlisted options" {
    run validate_socat_opts "reuseaddr,fork,backlog=128,bind=10.0.0.1,keepalive"
    [ "$status" -eq 0 ]
}
...
# --- S-03: structural IPv6 validation + hostname length cap -----------

@test "validate_hostname: rejects IPv6 group with 5 hex digits" {
    run validate_hostname "12345::"
    [ "$status" -eq 1 ]
}

@test "validate_hostname: rejects IPv6 with two '::' compressions" {
    run validate_hostname "1::2::3"
    [ "$status" -eq 1 ]
}

@test "validate_hostname: rejects triple-colon run" {
    run validate_hostname ":::"
    [ "$status" -eq 1 ]
}

@test "validate_hostname: rejects IPv6 with fewer than 8 groups and no '::'" {
    run validate_hostname "1:2:3"
    [ "$status" -eq 1 ]
}

@test "validate_hostname: accepts bare '::' (all-zeros)" {
    run validate_hostname "::"
    [ "$status" -eq 0 ]
}

@test "validate_hostname: rejects hostname exceeding 253 characters" {
    local long
    long="$(printf 'a%.0s' {1..251}).co"   # 254 chars total
    run validate_hostname "${long}"
    [ "$status" -eq 1 ]
}

# =====================================================================
# P-03: access-control validators (source range + tcpwrap name)
# =====================================================================

@test "validate_source_range: masks IPv4 host bits" {
    run validate_source_range "10.1.2.3/8"
    [ "$status" -eq 0 ]
    [ "$output" = "10.0.0.0/8" ]
}

@test "validate_source_range: masks IPv4 /24 host bits" {
    run validate_source_range "192.168.1.55/24"
    [ "$status" -eq 0 ]
    [ "$output" = "192.168.1.0/24" ]
}

@test "validate_source_range: renders and masks IPv6 (bracketed)" {
    run validate_source_range "2001:db8:abcd:1234::/48"
    [ "$status" -eq 0 ]
    [ "$output" = "[2001:db8:abcd:0:0:0:0:0]/48" ]
}

@test "validate_source_range: accepts default route 0.0.0.0/0" {
    run validate_source_range "0.0.0.0/0"
    [ "$status" -eq 0 ]
    [ "$output" = "0.0.0.0/0" ]
}

@test "validate_source_range: rejects IPv4 prefix > 32" {
    run validate_source_range "10.0.0.0/33"
    [ "$status" -eq 1 ]
}

@test "validate_source_range: rejects IPv6 prefix > 128" {
    run validate_source_range "2001:db8::/129"
    [ "$status" -eq 1 ]
}

@test "validate_source_range: rejects malformed CIDR" {
    run validate_source_range "notacidr"
    [ "$status" -eq 1 ]
}

@test "validate_source_range: rejects missing prefix" {
    run validate_source_range "10.0.0.0"
    [ "$status" -eq 1 ]
}

@test "validate_source_range: rejects octet > 255" {
    run validate_source_range "999.0.0.0/8"
    [ "$status" -eq 1 ]
}

@test "validate_tcpwrap_name: accepts a simple name" {
    run validate_tcpwrap_name "socat"
    [ "$status" -eq 0 ]
}

@test "validate_tcpwrap_name: accepts dots, underscores, hyphens" {
    run validate_tcpwrap_name "my-daemon_1.svc"
    [ "$status" -eq 0 ]
}

@test "validate_tcpwrap_name: rejects a name with a space" {
    run validate_tcpwrap_name "bad name"
    [ "$status" -eq 1 ]
}

@test "validate_tcpwrap_name: rejects empty name" {
    run validate_tcpwrap_name ""
    [ "$status" -eq 1 ]
}

@test "build_filter_opts: assembles range and tcpwrap fragments" {
    run build_filter_opts "10.0.0.0/8" "socat"
    [ "$status" -eq 0 ]
    [ "$output" = ",range=10.0.0.0/8,tcpwrap=socat" ]
}

@test "build_filter_opts: empty when no filters requested" {
    run build_filter_opts "" ""
    [ "$status" -eq 0 ]
    [ -z "$output" ]
}
