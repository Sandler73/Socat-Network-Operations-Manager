...
# =====================================================================
# F-03: mode_listen primary listener --capture wiring
# Previously capture_logfile was declared but never assigned on the
# primary listen path, so --capture produced no capture file (output
# fell through to the error log). This exercises the mode end-to-end.
# =====================================================================

@test "listen --capture: primary listener creates a 0600 capture log (F-03)" {
    export SOCAT_STUB_SLEEP=30
    run bash "${TEST_TMPDIR}/socat_manager.sh" listen --port 8080 --capture --name cap-primary
    [ "$status" -eq 0 ]

    # A capture file for the primary tcp4 listener must now exist.
    local f
    f="$(ls "${LOG_DIR}"/capture-tcp4-8080-*.log 2>/dev/null | head -1)"
    [ -n "$f" ]
    [ -f "$f" ]

    # It must be created with restrictive permissions.
    run stat -c '%a' "$f"
    [ "$output" = "600" ]
}

# =====================================================================
# F-06: forward --logfile overrides the capture path
# Previously --logfile wrote an unused global and was silently ignored.
# =====================================================================

@test "forward --capture --logfile: honors the custom capture path (F-06)" {
    export SOCAT_STUB_SLEEP=30
    local custom="${LOG_DIR}/custom-forward-capture.log"
    run bash "${TEST_TMPDIR}/socat_manager.sh" forward \
        --lport 8080 --rhost 10.0.0.1 --rport 80 --capture --logfile "$custom" --name fwd-cap
    [ "$status" -eq 0 ]

    # The user-specified path must be the one created (0600).
    [ -f "$custom" ]
    run stat -c '%a' "$custom"
    [ "$output" = "600" ]

    # And the auto-generated default path must NOT have been used instead.
    run bash -c "ls '${LOG_DIR}'/capture-tcp4-8080-10.0.0.1-80-*.log 2>/dev/null | wc -l"
    [ "$output" = "0" ]
}
