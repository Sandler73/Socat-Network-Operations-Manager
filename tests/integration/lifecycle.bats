...
# =====================================================================
# F-07: unconfirmed death must retain the session file and report failure
# Previously the session file was removed unconditionally and the return
# code was always 0, orphaning a process that could not be confirmed dead.
# =====================================================================

@test "_stop_session: retains session file and returns non-zero when death unconfirmed (F-07)" {
    # Register a session whose process cannot be confirmed dead. Port 0 skips
    # the port-based cleanup path so the outcome hinges solely on the PID check.
    session_register "eeee0007" "stuck-svc" "424242" "424242" \
        "listen" "tcp4" "0" "socat X Y" "" ""
    local sf="${SESSION_DIR}/eeee0007.session"
    [ -f "$sf" ]

    # Simulate a process that never dies: -0 always reports alive, signals no-op.
    kill() {
        if [[ "$1" == "-0" ]]; then return 0; fi
        return 0
    }

    run _stop_session "eeee0007"

    unset -f kill

    # Death could not be confirmed → non-zero return.
    [ "$status" -ne 0 ]
    # Session file retained so the session stays visible and stoppable on retry.
    [ -f "$sf" ]
}
